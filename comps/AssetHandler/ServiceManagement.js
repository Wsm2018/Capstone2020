...
export default function ServiceManagement(props) {
  ...
  useEffect(() => {
    if (selectedService != null) {
      ...
      for (let i = 0; i < users.length; i++) {
        if (users[i].services) {
          var found = users[i].services.filter(
            (s) => s === selectedService.service.id
          );
          if (found.length > 0) {
            temp.push(users[i]);
          }
  ...
  const handleDB = async () => {
    ...
    if (showAdd) {
      ...
      var s = await db
        .collection("services")
        .doc(selectedService.service.id)
        .get();
      var serv = s.data();
      ...
      Alert.alert(
        "Delete " + selectedService.service.name + " ?",
        "",
        [
          {
            text: "Yes",
  ...
  const color = (day, hour) => {
    ...
    if (selectedService) {
      ...
      found = serviceWorkHoursDays
        .filter((f) => f.service.id == selectedService.service.id)[0]
        .workingDays.filter((d) => d.day == day)[0]
        .hours.filter((t) => t == hour);
    } else {
      found = weekDays.current
        .filter((w) => w.day == day)[0]
  ...
  const addDay = (day, hour) => {
    if (selectedService) {
      ...
      var found = serviceWorkHoursDays
        .filter((f) => f.service == selectedService.service)[0]
        .workingDays.filter((d) => d.day == day)[0]
        .hours.filter((t) => t == hour);
      if (found.length > 0) {
        var tempHours = [];
        for (let i = 0; i < serviceWorkHoursDays.length; i++) {
          if (serviceWorkHoursDays[i].service == selectedService.service) {
            for (
              let k = 0;
              k < serviceWorkHoursDays[i].workingDays.length;
              k++
            ) {
              if (serviceWorkHoursDays[i].workingDays[k].day == day) {
                for (
                  let j = 0;
                  j < serviceWorkHoursDays[i].workingDays[k].hours.length;
                  j++
                ) {
                  if (serviceWorkHoursDays[i].workingDays[k].hours[j] != hour) {
                    tempHours.push(
                      serviceWorkHoursDays[i].workingDays[k].hours[j]
                    );
                  }
                }
  ...
  const manageWorker = async (status, u) => {
    const manageServiceWorkers = firebase
      .functions()
      .httpsCallable("manageServiceWorkers");
  ...
  const filterWorkers = () => {
    ...
    for (let i = 0; i < users.length; i++) {
      if (users[i].services && users[i].role == "services employee") {
        var check = users[i].services.filter(
          (s) => s == selectedService.service.id
        );
        if (check.length == 0) {
          temp.push(users[i]);
          ...
          ? services.map((s) => (
              <TouchableOpacity
                ...
                style={{
                  ...
                  padding: 4,
                  backgroundColor:
                    selectedService && selectedService.service == s
                      ? "#2E9E9B"
                      : "white",
                  borderColor: "#2E9E9B",
                  borderWidth: 1,
      ...
      {selectedService && selectedService.service ? (
        <View style={styles.three}>
          <View style={{ flexDirection: "row", width: "100%" }}>
            ...
            >
              {selectedService.service.name} Details
            </Text>
            ...
            <MaterialCommunityIcons
              ...
              onPress={
                () =>
                  ...
                  setShowAdd(false) ||
                  setMaxBookings(
                    parseInt(selectedService.service.maxBookings)
                  ) ||
          ...
          >
            <View>
              ...
              <View style={{ flexDirection: "row" }}>
                ...
                <Text style={styles.listDetails}>
                  {selectedService.service.price} QR
                </Text>
              </View>
              ...
              <View style={{ flexDirection: "row" }}>
                ...
                <Text style={styles.listDetails}>
                  {selectedService.service.maxBookings}
                </Text>
              </View>
              <View style={{ flexDirection: "row" }}>
                <Text style={styles.listNames}>Rating</Text>
                <Text style={styles.listDetails}>
                  {selectedService.service.rating}
                </Text>
              </View>
            </View>
          </View>
              ...
              <View>
                {workers.map((w) => (
                  ...
                  >
                    ...
                    >
                      <Text style={styles.listDetails2}>{w.email}</Text>
                    </TouchableOpacity>
                  </View>
                ))}
              </View>
            ...
            <View style={styles.modalView}>
              ...
              {selectedService.workingDays.map((w) => (
                <View style={{ width: "100%" }}>
                  ...
                  >
                    <TouchableOpacity
                      style={{ margin: "1%", flexDirection: "row" }}
                      onPress={() =>
                        isCollapsed == w.day
                          ? setIsCollapsed()
                          : setIsCollapsed(w.day)
                      }
                    >
                      <Text
                        style={{
          ...
          <View style={styles.centeredView}>
            <View style={styles.modalView3}>
              ...
              <View style={styles.info}>
                ...
                />
                <Text style={{ width: "80%", marginLeft: "3%" }}>
                      ...
                      >
                        <View style={{ flexDirection: "row" }}>
                          <MaterialCommunityIcons
                            ...
                            color={"grey"}
                          />
                          <Text style={{ width: "90%", marginLeft: "3%" }}>
                            {f.displayName}
                          </Text>
                        </View>
        ...
        <View style={styles.two3}>
          <View style={{ flexDirection: "row", width: "100%" }}>
            ...
            >
              {showAdd
                ? "Add New Service"
                : "Edit " + selectedService.service.name}
            </Text>
            <View
              style={{
                flexDirection: "row",
          ...
          >
            ...
            {week.map((w) => (
              ...
              >
                <TouchableOpacity
                  onPress={() => (collaps == w ? setCollaps() : setCollaps(w))}
                      ...
                      >
                        {icons.map((i, index) => (
                          ...
                          >
                            <TouchableOpacity
                              onPress={() =>
                                setServiceIcon(i) || setShowIcons(false)
                              }
                            >
                              <MaterialCommunityIcons
                                name={i}
